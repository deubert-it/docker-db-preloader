    ...
    stages {
        ...
        stage('Wait for import completion') {
            steps {
                ...
                sh 'docker-compose -f docker-compose.01-import.yml -p ${JOB_NAME}_import exec -T db bash -c "/tmp/wait-for-it.sh --timeout=$IMPORT_TIMEOUT_SECONDS --host=localhost --port=3306 -- /tmp/save-dbinfo.sh"'
